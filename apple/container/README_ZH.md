...
**核心思路**：不像 Docker Desktop 那样运行一个共享 Linux VM 来托管所有容器，而是 **每个容器运行在独立的轻量级虚拟机中**。
...
| **OCI 兼容** | 读取和生成标准 OCI 镜像，可与 OCI registry 及兼容工具互操作 |
...
container image pull       # 拉取镜像
container image push       # 推送镜像
container machine          # 管理持久化 Linux VM
...
| 资源模型 | 为共享 Linux VM 配置内存上限 | 每个轻量 VM 根据其容器工作负载使用内存 |
| 文件共享 | 将选定的主机目录共享给 Linux VM | 只把每个容器需要的主机数据挂载到其 VM |
...
- **官方支持**：macOS 26；macOS 15 可以运行，但存在官方记录的限制，且不会维护仅在旧版 macOS 上出现的问题
- **源码构建**：最低 macOS 15、推荐 macOS 26，并将 Xcode 26 设为当前开发环境
...
1. **正式支持 macOS 26。** macOS 15 可以运行但存在限制；无法在 macOS 26 复现的问题通常不会处理
