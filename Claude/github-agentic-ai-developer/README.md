...
This module is the one this note read unit-by-unit. Its throughline: **an agent should be scoped to specific SDLC stages, given a task contract instead of an open-ended goal, and routed entirely through pull requests so that GitHub's own mechanisms, rather than the agent's judgment, decide what gets accepted.**
...
### Use pull requests to enforce rules as well as to collaborate
...
- **Reliability assumes failure.** Bounded retries for transient check failures, escalation to a human after a check fails twice (with what failed, what was tried, and a suggested next step), rollback readiness for high-risk changes, and least-privilege workflow permissions (e.g. `contents: read`, `pull-requests: write`, and nothing broader by default).
