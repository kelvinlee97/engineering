...
fetched: 2026-09-25T17:43:52Z
title: What a task costs on Opus 5.5
---
- - - - - - - - - What a task costs on Opus 5.5 / claude.dev Blog- - 
[H]HOME[D]DOCS[T]TERMINAL
Try Claude Code 

[H]HOME[D]DOCS[T]TERMINAL

Playbooks# What a task costs on Opus 5.5
...
TREE
└The cost of a task, and the cost of a retry└What does a task cost?└What changed in Opus 5.5└The same tasks side-by-side└Tips for maximizing the value of your session└Measure it yourself└Keep in mind
░░░░░░░░░░░░░░░░░░░░░░░░░░░░00%
PRESS ↑ / ↓ TO SCROLL
SHARE
X.comLinkedInEmailCopy URL
...
FIG AAPI list price per million tokens.
...
Same tokens on Opus 5On Opus 5.5
$3.50at Opus 5 API list prices

- Read from cache 2.0M$1.00
- Fresh input 200K$1.00
- Output 60K$1.50
- Total, Opus 5$3.50

FIG BIllustrative session with the same tokens on both models, so this is the price change alone.
...
Typical Claude Code sessionSmall editLarge refactorLong agentic run
...
Token change on Opus 5.5 (your assumption)0%
Leave at 0% for price alone. Opus 5.5 can use more or fewer tokens than Opus 5, so measure it on your own tasks.

Opus 5, these tokens$3.50$770 a month
Opus 5.5, these tokens$2.40$528 a month
Change at API list prices−31%$242 less a month, from price alone

Opus 5, these tokens: $3.50. Opus 5.5, these tokens: $2.40. Change at API list prices: −31%, $242 less a month, from price alone.
FIG CA month is 22 working days. API list prices. On a Pro, Max or Team plan, this is not a bill. No batch or volume discounts, and cache writes are not counted (see caching).
...
lowmechanicalmediumeverydayhighif it stallsxhighhard problemsmaxone session
...
Fast mode runs Opus 5.5 up to 2.5 times faster, at twice the standard price: $8 per million input tokens and $40 per million output. On a Claude subscription it's billed to usage credits, not your plan limits. The first request after you turn it on pays the fast-mode input price on the whole conversation, uncached. So turn it on at the start of a session, not deep into one.
 ...
 | Cache lifetime | In Claude Code, one hour on a subscription; five minutes on an API key or cloud provider, or once a subscription draws on usage credits. Each hit resets the lifetime. The API offers a one-hour cache at a higher write price. Costs docs (Claude Code lifetime) and prompt caching docs
 | Cache writes | 1.25× the input price for a five-minute cache and 2× for a one-hour cache, on today's API pricing. Pricing docs
 | Thinking | Billed as output tokens, including thinking that isn't returned to you. Extended thinking docs
 | Effort | Applies to every output token: text, tool calls and thinking. Effort docs
 | Auto-compaction | Runs as the context window nears its limit. /autocompact with a token count sets the threshold. Context window docs
 | Subagents and agent teams | A subagent works in its own context window and returns a summary. Agent teams in plan mode use about 7× the tokens of a standard session. Context window and costs docs
 | Batch API | Half price on input and output tokens for requests sent through the Message Batches API. Batch processing docs
 | Typical spend today | Average: about $13 per developer per active day across enterprise deployments. For 90% of users, cost stays below $30 per active day. Both figures are for current models. Public docs, costs page (enterprise average and 90% bound)
...
- Run the same task twice. Opus 5.5 needs Claude Code v2.1.280 or later, so run claude update first. Pick something from your backlog, not a toy example. Use /model to switch between Opus 5 and Opus 5.5. Note turns, output tokens and cost for each run. Do three or four tasks before you draw a conclusion.
...
Related posts
ALLAGENTSENGINEERINGPLAYBOOKSSKILLS

Sep 23, 2026How we made claude.ai 3x faster in two weeks15 minSep 22, 2026Getting the most out of Opus 5.5 in Claude and Claude Code9 minJul 24, 2026The new rules of context engineering for Claude 5 generation models7 minJun 03, 2026Lessons from building Claude Code: How we use skills11 minJun 02, 2026A harness for every task: dynamic workflows in Claude Code11 min
LOAD MORE
