...
> ACM 本质是一台证书生命周期机器，而不只是证书仓库：它签发或导入证书，通过验证证明域名所有权，并且——仅对自己签发的证书——永远自动续期这份证明。

本文主要回答三个问题：

1. 证书要经历怎样的生命周期，自动化在哪里停止？
2. 证书创建时就固定下来的属性有哪些（区域、是否可导出）？
3. 证书卡住或失败时如何排查？
...
flowchart LR
    accTitle: ACM 证书生命周期
    accDescr: 证书要么向 ACM 申请，要么导入。申请的证书要经过验证、变为已签发状态，之后永远自动续期并重新验证。导入的证书立即可用，但从不自动续期。
    R[申请公有证书] --> V{DNS 或邮件<br/>验证}
    V -- 验证通过 --> I[已签发]
    I --> N[自动续期 +<br/>自动重新验证]
    N --> I
    M[导入第三方证书] --> I2[立即已签发]
...
ACM 签发的证书一旦通过验证即可自我维持。导入的证书跳过验证步骤，但过期前需要手动轮换。
