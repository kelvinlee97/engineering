...
```mermaid
flowchart TD
    accTitle: Amazon DocumentDB 集群架构
    accDescr: 集群包含一个主实例和最多 15 个副本，共享同一个在三个可用区间 6 副复制的集群卷。读取端点在副本间负载均衡应用的读请求，写入则发往主实例。
    App[应用] -->|写入| Primary[主实例]
    App -->|通过读取端点读取| Reader[读取端点]
    Reader --> R1[副本]
    Reader --> R2[副本]
    Primary --> Vol[共享集群卷<br/>6 副复制，3 个可用区]
    R1 --> Vol
    R2 --> Vol
```
