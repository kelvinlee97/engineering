...
> AMS 相当于以服务形式提供的运维团队：你的变更走受控的请求流程，而 AMS 自己的基线在背后持续处理监控、补丁、安全和备份。
...
flowchart LR
    accTitle: AMS 变更请求流程
    accDescr: 客户通过 AMS 控制台提交变更请求，AMS 依据策略和护栏评估，通过后实施变更；同时 AMS 拥有的基线在后台持续处理监控、补丁、安全和备份。
    Y[你的团队] -->|提交变更请求| C[AMS 控制台]
    C --> P{策略与<br/>护栏检查}
    P -- 通过 --> I[AMS 实施变更]
    P -- 拒绝 --> Y
    I --> E[你的 AWS 环境]
    G[AMS 基线：<br/>监控、补丁、<br/>安全、备份] --> E
```
