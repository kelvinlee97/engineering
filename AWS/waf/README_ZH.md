...
> AWS WAF 是 Web 应用防火墙，监控发往受保护资源的 HTTP(S) 请求，并根据规则（IP、查询字符串、请求头、请求体）控制访问。
...
flowchart LR
    accTitle: WAF 请求评估流程
    accDescr: 入站 HTTP 请求按优先级顺序与 Web ACL 中规则组的规则逐一比对，第一条命中规则的动作（allow、block 或 count）决定响应；没有规则命中时使用默认动作。
    R[入站 HTTP 请求] --> W[Web ACL：<br/>按优先级排列的规则组]
    W --> M{规则命中?}
    M -- Allow --> A[转发到受保护资源]
    M -- Block --> B[HTTP 403 或自定义响应]
    M -- Count --> C[仅记录日志，继续评估]
    M -- 未命中 --> D[默认动作]
```
