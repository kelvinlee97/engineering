...
> Auto Scaling Group 是一个能自我修复的容量边界：它从不会低于最小值或高于最大值启动实例，扩缩策略在这个边界内移动期望容量，而健康检查会持续替换任何脱离健康状态的实例，不论失败原因是什么。

本文主要回答两个问题：

1. 是什么把实例数量固定在 min/max 之间？是什么在这个边界内推动期望容量变化？
2. 不健康的实例是如何在无需人工干预的情况下被替换的？

## 全景图

```mermaid
flowchart LR
    accTitle: Auto Scaling Group 容量控制
    accDescr: 期望容量受组的最小和最大值约束。由 CloudWatch 指标或计划驱动的扩缩策略在这些边界内调整期望容量。健康检查持续监控实例；不健康的实例会被终止并替换以恢复期望容量；启动模板变更时，实例刷新执行滚动替换。
    B["边界：min ≤ desired ≤ max"] --> D[期望容量]
    P[扩缩策略：<br/>目标跟踪 / 步进 / 定时] --> D
    D --> L[从启动模板<br/>启动实例]
    L --> H{健康检查<br/>通过?}
    H -- 否 --> T[终止并替换]
    T --> L
...
健康驱动的替换和策略驱动的扩缩是两条独立的循环，但都汇入同一个基于启动模板的实例创建流程。
