...
> AWS Config 只有在资源变更发生时记录器恰好在运行，才会知道这个资源：记录、投递、规则评估是依次进行的三个独立步骤，所以一个资源完全真实存在，却仍可能对 Config、对 S3 历史、对合规规则不可见——只要前面任何一步被关闭或未获授权。

本文主要回答一个问题：

1. 当资源的历史、投递或合规状态缺失时，这三个先后步骤中到底是哪一步出了问题？

## 全景图

```mermaid
flowchart LR
    accTitle: AWS Config 记录与评估流水线
    accDescr: 配置记录器必须处于运行状态才能把资源变更捕获为配置项。该配置项被投递到 S3 桶作为历史和快照，并触发 SNS 通知。另外，托管或自定义规则会评估已记录的资源类型并报告合规或不合规，聚合器可以把这一切集中到跨账户、跨区域的视图。
    Chg[资源变更] --> R{该资源类型的<br/>记录器在运行?}
    R -- 否 --> Miss[变更对 Config<br/>不可见]
    R -- 是 --> CI[配置项]
    CI --> S3[S3：历史 + 快照]
    CI --> SNS[SNS 通知]
    CI --> Rules{是否有规则<br/>覆盖该类型?}
    Rules -- 是 --> Comp[合规 / 不合规]
    CI --> Agg[聚合器：<br/>跨账户/区域视图]
```

在记录器停止时删除资源是一个常见的漏洞：没有运行中的记录器，删除事件本身永远不会被捕获，留下"仍然存在"的过期结果。
