...
```mermaid
flowchart LR
    accTitle: AWS Glue 数据集成流程
    accDescr: 爬虫连接数据源、推断 schema 并填充 Data Catalog。在 Glue Studio 中可视化构建或由触发器/工作流调度的 ETL 作业读写数据并使用该目录，目录可被 Athena、EMR 和 Redshift Spectrum 查询。
    Src[数据源] --> Cr[爬虫]
    Cr --> Cat[Data Catalog]
    Cat --> Job[ETL 作业<br/>Spark / Ray]
    Job --> Cat
    Trig[触发器 / 工作流] --> Job
