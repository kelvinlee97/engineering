...
> CloudFormation 把基础设施当成由差异比对驱动的一个整体，而不是直接下达命令：每次更新——无论安不安全——本质都是"先生成变更集，再执行它"，而堆栈的事件日志是查明失败原因唯一可靠的地方。

本文主要回答两个问题：

1. 从"我改了模板"到"变更已生效"，安全路径是怎样的？
2. 堆栈创建或更新失败时，应该先看哪里？
...
```mermaid
flowchart LR
    accTitle: CloudFormation 变更集工作流
    accDescr: 模板变更生成一个变更集，即与已部署堆栈相比新增、修改、删除的预览。执行变更集会应用它，产生一系列堆栈事件；这些事件中第一个 CREATE_FAILED 或 UPDATE_FAILED 就是失败的根因。
    T[更新后的模板] --> CS[创建变更集：<br/>与已部署堆栈比对]
    CS --> R[审查：新增、<br/>修改、删除]
    R --> E["执行变更集"]
    E --> EV[堆栈事件]
    EV --> F{是否有 FAILED 事件?}
    F -- 是 --> RC[第一个 FAILED 事件<br/>= 根因]
    F -- 否 --> OK[堆栈已更新]
```

变更集这一步的存在，正是为了在应用之前先审查差异——直接跳到 `update-stack` 只是省略了这道安全检查，而不是绕过底层机制本身。
