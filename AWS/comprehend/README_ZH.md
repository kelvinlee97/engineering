...
> Comprehend 本质是一组预训练 NLP 检测器（实体、情感、PII、语法……）外加一层可选的 AutoML：优先使用 `Detect*` API，只有当你的类别或术语足够特定、预训练模型无法命名时，才需要构建自定义分类器或实体识别器。

本文主要回答一个问题：

1. 这个任务应该用内置的 `Detect*` API，还是需要自定义分类器/识别器？
