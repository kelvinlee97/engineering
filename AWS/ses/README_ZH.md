...
> Amazon Simple Email Service（Amazon SES）是可扩展的电子邮件平台，用于发送事务性邮件（订单确认、密码重置）、营销邮件（优惠、新闻简报），也支持接收邮件。 你可以通过 SES API、SMTP 接口或 AWS SDK 发送邮件，将收到的邮件路由到 S3、SNS 或 Lambda。
