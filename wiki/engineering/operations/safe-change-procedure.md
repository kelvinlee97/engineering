...
3. **Validate before applying.** `nginx -t` or `openresty -t` before any reload; for a release, `npm ci`, tests, and `node --check` in the new release directory before switching.[^nginx-production-guide][^express-bff-deployment-guide]
