...
Claude Managed Agents is a hosted agent harness: Anthropic runs the agent loop, the sandbox, and tool execution, and your application sends events into a running session instead of implementing its own runtime. It was in beta at review time (2026-09-15); every request needs the `managed-agents-2026-04-01` header, which the SDK sets.[^claude-managed-agents]

## Building blocks

| Concept | What it is |
| --- | --- |
| Agent | Model, system prompt, tools, MCP servers, and [skills](agent-skills.md#what-a-skill-is); created once and referenced by ID |
...
As described in the documentation. You create an agent and an environment, start a session, and send events; Claude runs tools on its own and streams results back over server-sent events. Event history is stored server-side, so a session survives pauses, and you can steer or interrupt it with further events.
...
- Access was on by default for API accounts; MCP tunnels and "dreaming" were a narrower research preview.
- History, sandbox state, and outputs persist server-side, so it was not eligible for Zero Data Retention or HIPAA BAA coverage. Sessions and uploaded files can be deleted through the API.
...
- [Cloud environment](cloud-sessions.md#cloud-environment): the Claude Code counterpart for sandbox configuration.
