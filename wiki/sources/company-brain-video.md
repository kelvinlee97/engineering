---
type: Source Summary
title: Every company should have a brain (summary)
description: Summary of the legacy note on Garry Tan's talk about skills as an organization and a curated company memory.
tags: [ai-adoption, skills, memory, video]
sources:
  - id: company-brain-video
    resource: https://github.com/kelvinlee97/engineering/blob/main/YouTube/startup/every-company-should-have-a-brain--eBUyTS7SzV4/summary.md
    title: Every Company Should Have a Brain (video summary)
    author: human:kelvinlee97
    last_modified: 2026-09-24T14:48:10Z
generated: { by: claude-code/wiki-v1, at: 2026-09-25T05:45:00Z }
status: draft
---
A note in this repository, `YouTube/startup/every-company-should-have-a-brain--eBUyTS7SzV4/summary.md`, on Garry Tan's 21:08 AI Engineer talk. The note calls it an advocacy talk, not a controlled study.[^company-brain-video] See [Company brain](../engineering/ai-engineering/company-brain.md).

[^company-brain-video]: Every Company Should Have a Brain (video summary)
