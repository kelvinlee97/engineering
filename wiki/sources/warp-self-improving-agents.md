...
A note in this repository, `Claude/self-improving-agents/README.md`, on Anthropic's post *How Warp builds self-improving agents on Claude* (from a webinar with Warp's founder and Anthropic's Applied AI team) and Warp's related posts, reviewed on 2026-09-16.[^warp-self-improving-agents]
...
- A scheduled outer skill reads human feedback on an agent's work and opens a pull request editing the inner skill the agent reads.[^warp-self-improving-agents] See [Self-improving skill loop](../engineering/ai-engineering/self-improving-skill-loop.md).
- Improvements arrive as pull requests, so the team, not the agent, accepts them.[^warp-self-improving-agents] See [Agents propose, people and policy accept](../engineering/ai-engineering/propose-accept-boundary.md).
