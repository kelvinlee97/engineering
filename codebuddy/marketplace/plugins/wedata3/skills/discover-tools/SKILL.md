...
- `LOCAL_SECRET_ID` — must not be `your-secret-id`
- `LOCAL_SECRET_KEY` — must not be `your-secret-key`
...
| Issue | Solution |
|-------|----------|
| Authentication error | Verify `LOCAL_SECRET_ID` and `LOCAL_SECRET_KEY` in `.env.local` are correct |
