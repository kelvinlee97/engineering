...
- `LOCAL_SECRET_ID` — must not be `your-secret-id` or empty
- `LOCAL_SECRET_KEY` — must not be `your-secret-key` or empty

**Auto-filled by platform (should already have real values):**
- `WEDATA_WORKSPACE_ID`, `WEDATA_APP_KEY`, `WEDATA_SDK_ENDPOINT`

If AK/SK are missing or placeholder → tell the user to run the **wedata-quickstart** skill first, or fill them manually. Do NOT proceed.
...
1. ✅ Verified `.env.local` — AK/SK filled; platform fields auto-filled
