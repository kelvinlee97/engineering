...
> 现代化问题的关键在于运维方式，而不是组件名称：只要有证据证明它们无状态、以不可变制品发布且可观测，BFF、网关和下游服务今天完全可以保持不变，同时仍然是“现代”的——底层平台是最后才需要做的决定，而不是第一个。

```mermaid
flowchart LR
    accTitle: 常见的浏览器到下游请求路径
    accDescr: 浏览器请求到达边缘网关，网关转发给 Node.js BFF，BFF 再调用 OpenResty/Lua 网关功能或下游服务。
    B[浏览器] --> E[边缘网关]
    E --> N[Node.js BFF]
    N --> D[OpenResty/Lua<br/>或下游服务]
```
...
flowchart TD
    accTitle: 渐进式目标架构分层
    accDescr: Internet 流量依次经过 CDN/WAF 与托管负载均衡、网关策略、无状态 Node.js BFF 副本、OpenResty/Lua 网关功能或已批准下游服务、托管数据与身份服务。每一跳都产生可关联的日志、指标、追踪与发布元数据，CI 流水线产出已测试不可变制品，经渐进发布并配合受监控回滚。
    I[Internet] --> C[CDN/WAF 与<br/>托管负载均衡]
    C --> G[网关策略<br/>Gateway API 或同类能力]
    G --> N[无状态 Node.js<br/>BFF 副本]
    N --> O[OpenResty/Lua 网关功能<br/>与已批准下游服务]
    O --> M[托管数据与<br/>身份服务]
    CI[CI: 已测试不可变制品] --> PR[渐进发布] --> RB[受监控回滚]
```

每一跳都携带可关联的日志、指标、追踪和发布元数据。
