...
> 失败请求会经过一条固定链路——网关、PM2/BFF 进程、路由/应用逻辑、下游依赖、主机资源——下面 10 类事故分别对应其中某一环出问题的常见方式；先找到第一个断开的环节，再采取任何动作。
...
flowchart TD
    accTitle: 事故分类判断流程
    accDescr: 从用户请求失败出发，依次检查是否到达网关、网关能否连到 BFF 的 healthz、BFF 是否接收请求、BFF 能否连到下游，以及资源是否健康；任一环节若刚发布过新版本，则转向比较版本并使用批准回滚。
    U[用户请求失败] --> G{是否到达网关?}
    G -- 否 --> G1[DNS/LB/TLS/网关负责人]
    G -- 是 --> H{网关可达<br/>BFF /healthz?}
    H -- 否 --> H1[PM2、端口、release、主机]
    H -- 是 --> A{BFF 是否<br/>接收请求?}
    A -- 否 --> A1[路由、授权、<br/>配置、应用日志]
    A -- 是 --> D{BFF 可达下游?}
    D -- 否 --> D1[DNS/网络/下游负责人]
    D -- 是 --> R{资源是否健康?}
    R -- 否 --> R1[容量/保留策略/<br/>批准的缓解动作]
    R -- 是 --> C{刚发布过?}
    C -- 是 --> C1[比较版本并<br/>使用批准回滚]
```
